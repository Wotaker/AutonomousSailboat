...
###################################################################################################
# Standard import
from math import pi, sin, cos, atan, atan2, sqrt, copysign, radians
...
param_dict = yaml.safe_load(sim_params)
...
HEIGHT_BUOYANCY = param_dict['boat']['height_bouyancy']
...
YAW_TIME_CONSTANT = param_dict['boat']['yaw_timeconstant']
...
# Hydrostatic force
HYDROSTATIC_EFF_X = HEIGHT_BUOYANCY + (WATER_DENSITY / MASS) * GEOMETRICAL_MOI_X
HYDROSTATIC_EFF_Y = HEIGHT_BUOYANCY + (WATER_DENSITY / MASS) * GEOMETRICAL_MOI_Y
...
DAMPING_INVARIANT_YAW = -(MOI_Z / YAW_TIME_CONSTANT)
...
def calculate_sail_force(roll, wind, sail_angle):
    ...
    propulsion = (2 * pi * eff_aoa * sin(wind.angle)
                  - (friction + (4 * pi * eff_aoa ** 2 * separation) / SAIL_STRETCHING)
                  * cos(wind.angle)) * SAIL_AREA * pressure

    transverse_force = (-2 * pi * eff_aoa * cos(wind.angle)
                        - (friction + (4 * pi * eff_aoa ** 2 * separation) / SAIL_STRETCHING)
                        * sin(wind.angle)) * SAIL_AREA * pressure
...
def calculate_lateral_force(vel_x, vel_y, roll, speed):
    ...
    return LateralForce(
        ...
        y=(1 - separation) * (tmp * sin(aoa) - 2 * pi * eff_aoa * cos(
            aoa)) * pressure * LATERAL_AREA + separation * separated_transverse_force), separation
...
def calculate_wave_influence(pos_x, pos_y, yaw, wave, tm):
    ...
    factor = -wave.amplitude * cos(frequency * tm - k.x * pos_x - k.y * pos_y)
    gradient_x = k.x * factor
    gradient_y = k.y * factor

    return WaveInfluence(
        height=wave.amplitude * sin(frequency * tm - k.x * pos_x - k.y * pos_y),
...
def calculate_hydrostatic_force(pos_z, roll, pitch, wave_influence):
    ...
    return HydrostaticForce(
        ...
        z=force
    ), HYDROSTATIC_EFF_Y * sin(
        pitch + atan(wave_influence.gradient_x)
    ), HYDROSTATIC_EFF_X * (
        -sin(roll - atan(wave_influence.gradient_y))
    )
...
def solve(current_time, boat):
    ...
    rudder_angle_reference = None
    sail_angle_reference = None
    if actor_dynamics:
    ...
    # For force calculations needed values
    speed = sqrt(vel_x ** 2 + vel_y ** 2)  # + vel_z**2)
    wave_influence = calculate_wave_influence(pos_x, pos_y, yaw, wave, current_time)
    ...
    delta_vel_x = delta_yaw * vel_y + (
            sail_force.x + lateral_force.x + rudder_force.x + damping.x + wave_impedance + hydrostatic_force.x
    ) / MASS

    delta_vel_y = (-delta_yaw) * vel_x + (
            (
                    sail_force.y + lateral_force.y + rudder_force.y
            ) * cos(roll) + hydrostatic_force.y + damping.y) / MASS

    delta_vel_z = ((
                           sail_force.y + lateral_force.y + rudder_force.y
                   ) * sin(roll) + hydrostatic_force.z - GRAVITY_FORCE + damping.z) / MASS
    ...
    delta_yaw_rate = (damping.yaw
                      ...
                      + lateral_force.y * (
                              DISTANCE_COG_KEEL_PRESSURE_POINT * (1 - lateral_separation)
...
###################################################################################################
# Simulate for a given time step
def step():
